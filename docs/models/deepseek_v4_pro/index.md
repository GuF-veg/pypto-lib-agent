...
| Expert parallelism | `--ep 2/4/8`; each rank always keeps 48 routed experts (`n_routed_experts // 8`), so the routing space shrinks to `48 * ep` and only EP8 covers all 384 Pro experts; real Flash requires EP8 with 32 routed experts per rank |
